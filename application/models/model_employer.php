...
class Model_employer extends CI_Model {
    ...
    public function get_denied($jobno)
    {
        $db1 = $this->load->database('local', TRUE);
        $query = $db1->query("SELECT *,DATE_FORMAT(datereceived, '%M %d %Y') as datereceived 
...
//    }
    ...
    public function change_status($what,$applicationid,$jobno)
    {
        ...
        $db1->query("UPDATE applications SET status = '$what', confirmed = 0 where applicationid = $applicationid");
        ...
        if($left <= 0)
        {
            $query2 = $db1->query("SELECT appid from applications WHERE status != 'Hired' and status!= 'Denied' and jobno = $jobno");
            return $query2->result_array();
        }
        else return 0;
        $db1->close();
    }
    public function change_status2($what,$appid,$jobno)
    {
        $db1 = $this->load->database('local', TRUE);
    ...
    public function change_employment($appid)
    {
     ...
     public function add_schedule1($scheduledate,$what, $venue, $contactperson, $contactno,$applicationid)
    {
        $db1 = $this->load->database('local', TRUE);
    ...
    public function add_scheduleslot($scheduleid, $starttime,$endtime)
    {
        ...
        $db1->query($sql2,array($scheduleid, $starttime,$endtime));
        $db1->close();
    }
